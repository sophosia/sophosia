...
/**
 * Keywords users are able to use in custom citekey rule
 */
export const citeKeyKeywords = [
  "author",
  "year",
  "title",
  "fullTitle",
 ...
 * @param {string} rule - The rule for generating the citation key (default: "author_year_title").
...
export function generateCiteKey(meta: Meta, rule: string): string {
  ...
  for (const keyword of citeKeyKeywords) parts[keyword] = "unknown";
  // publiser
  if (meta.publisher) {
    parts.publisher = meta.publisher.toLowerCase().replaceAll(" ", "_");
  }

  // journal
  if (meta["container-title"]) {
    parts.journal = meta["container-title"].toLowerCase().replaceAll(" ", "_");
  }
  ...
  if (meta.author && meta.author.length > 0) {
    ...
    parts.author = familyNames.join("_").replace(/[\s\-\._]/g, "_");
  }

  // year
  if (meta.issued) {
    parts.year = meta.issued["date-parts"][0][0].toString();
  }

  // uid
  parts.uid = `SP${db.nanoid}`;

  // default to use untranslated title if it exists
  const title = getTitle(meta, false);
  parts.fullTitle = title
    .toLowerCase()
    .split(" ")
    .join("_")
    .replace(/[,.:'"/?!~`\\|]/g, "") // remove all special characters
    .replace(/[\s\-\._]/g, "_");
  // title's first meaningful word
  parts.title = parts.fullTitle
    .split("_")
    .filter((w) => !["a", "an", "the", "on"].includes(w))[0];

  let citeKey = rule;
  for (const [key, val] of Object.entries(parts)) {
    citeKey = citeKey.replaceAll(key, val);
  }
  return citeKey;
}
