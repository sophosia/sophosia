...
import { Note, NodeType, Project } from "../database";
...
describe("graph.ts", () => {
  ...
  beforeAll(async () => {
    ...
    const note2 = await createNote(projects[2]._id, NodeType.MARKDOWN);
    console.log("note2._id", note2._id);
    notes.push((await addNote(note2)) as Note);

    const note1 = await createNote(projects[1]._id, NodeType.MARKDOWN);
    notes.push((await addNote(note1)) as Note);
    await updateForwardLinks(note1._id, [note2._id]);

    const note0 = await createNote(projects[0]._id, NodeType.MARKDOWN);
