...
export default class PDFApplication {
  ...
  isPageHistoryJump = false;
  pageHistoryMaxLen = 10;
  pageHistoryIndex = ref(this.pageHistoryMaxLen - 1);
  ...
  init(container: HTMLDivElement) {
    ...
    eventBus.on(
      ...
      async (e: {
        ...
        if (!this.isPageHistoryJump) {
          // if changing pages in the history
          // start an alternate history from that index
          if (this.pageHistoryIndex.value < this.state.pageHistory.length - 1) {
            this.state.pageHistory = this.state.pageHistory.slice(
              0,
              this.pageHistoryIndex.value + 1
            );
            this.pageHistoryIndex.value = this.pageHistoryMaxLen - 1;
          }
          // append page to history
          if (
            this.state.pageHistory[this.state.pageHistory.length - 1] !==
            e.pageNumber
          )
            this.state.pageHistory.push(e.pageNumber);
          // only store certain amount of history
          if (this.state.pageHistory.length > this.pageHistoryMaxLen)
            this.state.pageHistory.shift();
          // indicates we are at the latest history (current page)
          this.pageHistoryIndex.value = this.state.pageHistory.length - 1;
        }
        this.state.currentPageNumber = e.pageNumber;
        this.isPageHistoryJump = false;
  ...
  async loadState(projectId: string): Promise<PDFState | undefined> {
    try {
      let state = await db.get("SS" + projectId.slice(2));
  ...
  /**
   * Jump to next/previous history page
   * @param {number} indexDelta - Next history: +1, previous history: -1
   */
  jumpToPageHistory(indexDelta: number) {
    this.isPageHistoryJump = true;
    if (
      this.pageHistoryIndex.value + indexDelta < 0 ||
      this.pageHistoryIndex.value + indexDelta >
        this.state.pageHistory.length - 1
    )
      return;
    this.pageHistoryIndex.value += indexDelta;
    const pageNumber = this.state.pageHistory[this.pageHistoryIndex.value];
