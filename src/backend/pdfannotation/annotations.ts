...
export abstract class Annotation {
  ...
  enableDragToMove() {
    ...
    dom.ondragstart = (e) => {
      ...
      e.dataTransfer?.setData("dom", "dragging");
      // no ghost image when dragging
      // webkit can't handle dom manipulation in ondragstart function, have to settimeout
      setTimeout(() => {
        e.dataTransfer?.setDragImage(emptyImage, 0, 0);
      }, 0);
    };

    // in webkit, there is a ghost image flying back to original position
    // disable that using the dragover function
    dom.parentElement!.ondragover = (e) => {
      e.preventDefault();
    };
